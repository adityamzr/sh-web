...
<template>
  <section class="bg-sht-off-white py-20 sm:py-24 lg:py-28" aria-labelledby="latest-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      ...
      <div class="-mx-5 mt-10 flex snap-x snap-mandatory gap-5 overflow-x-auto px-5 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:mx-0 lg:grid lg:overflow-visible lg:px-0 lg:pb-0 lg:grid-cols-4 lg:gap-x-6">
        <article v-for="entry in latestEntries" :key="entry.title" class="group min-w-[82%] snap-start lg:min-w-0">
          ...
          <span class="mt-4 inline-flex text-sm font-semibold text-sht-olive transition-transform duration-300 group-hover:translate-x-1">Baca Artikel <span class="ml-1" aria-hidden="true">→</span></span>
        </article>
      </div>
    </div>
  </section>
