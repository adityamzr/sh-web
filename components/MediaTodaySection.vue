  ...
  <section id="hari-ini" class="bg-sht-off-white py-14 sm:py-24 lg:py-28" aria-labelledby="today-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      <div class="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p class="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.24em] text-sht-sage sm:text-xs"><span class="h-px w-8 bg-sht-sage" aria-hidden="true" />{{ t('SOROTAN') }}</p>
          <h2 id="today-heading" class="mt-3 font-hero text-4xl font-bold italic leading-[0.98] text-sht-olive-dark sm:text-5xl">{{ t('Sorotan Haramain') }}</h2>
          <p class="mt-4 max-w-xl text-base leading-relaxed text-sht-charcoal/70">{{ t('Catatan terbaru, kondisi lapangan, dan informasi yang relevan dari Makkah dan Madinah.') }}</p>
        </div>
        <NuxtLink :to="localePath('/hari-ini')" class="text-xs font-semibold uppercase tracking-[0.16em] text-sht-olive hover:text-sht-olive-dark sm:pb-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sht-gold">{{ t('Lihat Semua Update →') }}</NuxtLink>
      </div>
      ...
      <div class="mt-12 grid gap-8 lg:grid-cols-[minmax(0,1.65fr)_minmax(20rem,0.9fr)] lg:gap-10">
        ...
        </NuxtLink><div v-else class="flex min-h-[440px] flex-col justify-center p-8"><h3 class="font-hero text-3xl">{{ featuredUpdate.title }}</h3><p class="mt-4 text-sm">{{ featuredUpdate.description }}</p><NuxtLink v-if="locale === 'en'" to="/hari-ini" class="mt-6 underline">{{ t('Buka versi Indonesia') }}</NuxtLink></div></article>
