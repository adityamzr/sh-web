...
<template>
  <section class="bg-sht-off-white py-14 sm:py-28" aria-labelledby="madinah-close-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      ...
      <div class="mt-10 flex snap-x snap-mandatory gap-4 overflow-x-auto px-0 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:hidden" aria-label="Cerita Madinah lainnya"><template v-for="story in stories.slice(1)" :key="story.key"><NuxtLink :to="`/artikel/${story.key}`" class="min-w-[82%] snap-start rounded-3xl border border-sht-stone bg-white p-5"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-sht-sage">{{ story.category }}</p><h3 class="mt-3 font-hero text-xl font-bold not-italic leading-tight text-sht-olive-dark">{{ story.title }}</h3><p v-if="story.summary" class="mt-3 text-sm leading-relaxed text-sht-charcoal/70">{{ story.summary }}</p><span class="mt-5 inline-flex text-sm opacity-70 text-sht-olive">Baca catatan</span></NuxtLink></template></div>
    </div>
  </section>
</template>
