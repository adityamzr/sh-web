...
const { articles, pending: articlesPending, error: articlesError } = await useMediaArticles({ limit: 100, city: 'MAKKAH' })
const { settings, pending: settingsPending } = await useMediaPageSettings('makkah')
  ...
  <section class="bg-sht-off-white py-14 sm:py-28" aria-labelledby="makkah-close-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      <div class="max-w-3xl">
        ...
        <h2 id="makkah-close-heading" class="mt-4 font-hero text-4xl font-bold italic leading-tight text-sht-olive-dark sm:text-5xl">Makkah dari Dekat</h2>
        <p class="mt-4 max-w-2xl text-base leading-relaxed text-sht-charcoal/70">Cerita, kebiasaan, dan hal-hal praktis yang sering baru terlihat ketika Makkah dilihat lebih lama.</p>
      </div>
      ...
      <div v-else-if="!stories.length" class="mt-12">
        <CommonEmptyState title="Belum ada cerita Makkah" description="Cerita dari dekat akan muncul di sini setelah artikel tersedia." />
      </div>
      ...
      <template v-else>
        ...
        <div class="mt-10 flex snap-x snap-mandatory gap-4 overflow-x-auto px-0 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:hidden" aria-label="Cerita Makkah lainnya">
