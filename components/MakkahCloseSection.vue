...
const { articles } = useMediaArticles({ limit: 100, city: 'MAKKAH' }); const { settings } = useMediaPageSettings('makkah')
...
<template>
  <section class="bg-sht-off-white py-14 sm:py-28" aria-labelledby="makkah-close-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      <div class="max-w-3xl"><p class="text-xs font-semibold uppercase tracking-[0.22em] text-sht-sage">{{ t('CERITA & SUDUT LOKAL') }}</p><h2 id="makkah-close-heading" class="mt-4 font-hero text-4xl font-bold italic leading-tight text-sht-olive-dark sm:text-5xl">{{ t('Makkah dari Dekat') }}</h2><p class="mt-4 max-w-2xl text-base leading-relaxed text-sht-charcoal/70">{{ t('Cerita, kebiasaan, dan hal-hal praktis yang sering baru terlihat ketika Makkah dilihat lebih lama.') }}</p></div>
      ...
      <div class="mt-10 flex snap-x snap-mandatory gap-4 overflow-x-auto px-0 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:hidden" :aria-label="t('Cerita Makkah lainnya')"><template v-for="story in stories.slice(1)" :key="story.key"><NuxtLink :to="localePath(`/artikel/${story.slug}`)" class="min-w-[82%] snap-start focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sht-gold rounded-3xl border border-sht-stone bg-white p-5"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-sht-sage">{{ t(story.category) }}</p><h3 class="mt-3 font-hero text-xl font-bold not-italic leading-tight text-sht-olive-dark">{{ story.title }}</h3><p v-if="story.summary" class="mt-3 text-sm leading-relaxed text-sht-charcoal/70">{{ story.summary }}</p><span class="mt-5 inline-flex text-sm opacity-70 text-sht-olive">{{ t('Baca catatan') }}</span></NuxtLink></template></div>
    </div>
  </section>
</template>
