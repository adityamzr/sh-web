...
const { articles } = await useMediaArticles({ limit: 100 }); const { settings } = await useMediaHomeSettings()
...
const entries=computed(()=>{const ids=settings.value?.editorialArticleIds||[];const selected=ids.map(id=>articles.value.find(a=>a.id===id)).filter((a):a is NonNullable<typeof a>=>Boolean(a));const fallback=articles.value.filter(a=>!selected.some(x=>x.id===a.id));const editorial=[...selected,...fallback];const out=baseEntries.map(x=>({...x}));const primary=editorial[0],local=editorial[1];if(primary){out[0].title=primary.title;out[0].supporting=primary.excerpt;out[0].image=primary.image;out[0].alt=primary.imageAlt;out[0].href=`/artikel/${primary.slug}`}if(local){out[3].title=local.title;out[3].supporting=local.excerpt;out[3].image=local.image;out[3].alt=local.imageAlt;out[3].href=`/artikel/${local.slug}`}return out})
</script>
