...
const latestArticles = articles.filter((article) => article.city === 'madinah').slice(0, 4)
</script>
  ...
  <section class="bg-sht-off-white py-24 sm:py-28" aria-labelledby="madinah-latest-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      ...
      <div class="mt-10 hidden gap-x-5 gap-y-12 md:grid md:grid-cols-2 lg:mt-12 lg:grid-cols-4">
        <article v-for="article in latestArticles" :key="article.id" class="group min-w-0 snap-start md:flex md:flex-col"><NuxtLink :to="`/artikel/${article.slug}`" class="block focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sht-gold">
          ...
          <p class="mt-4 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-sht-sage">{{ article.category }} <span class="text-sht-stone">·</span> <span class="text-sht-charcoal/50">{{ article.publishedAt }}</span></p>
      ...
      <div class="-mx-5 mt-10 overflow-x-auto px-5 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:hidden" aria-label="Artikel terbaru dari Madinah">
        <div class="flex w-max snap-x snap-mandatory gap-4">
          <article v-for="article in latestArticles" :key="`mobile-${article.id}`" class="group w-[82vw] max-w-[21rem] flex-none snap-start"><NuxtLink :to="`/artikel/${article.slug}`" class="block focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sht-gold">
            <div class="overflow-hidden rounded-2xl bg-sht-stone"><img :src="article.image" :alt="article.imageAlt" loading="lazy" class="aspect-[4/3] w-full object-cover transition-transform duration-500 ease-out group-hover:scale-[1.02]" /></div>
            <p class="mt-4 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-sht-sage">{{ article.category }} <span class="text-sht-stone">·</span> <span class="text-sht-charcoal/50">{{ article.publishedAt }}</span></p>
