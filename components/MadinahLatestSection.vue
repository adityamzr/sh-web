    ...
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      <div class="flex items-end justify-between gap-6">
        <div>
          ...
          <h2 id="madinah-latest-heading" class="mt-4 font-hero text-4xl font-bold italic leading-tight text-sht-olive-dark sm:text-5xl">{{ t('Terbaru dari Madinah') }}</h2>
          <p class="mt-4 max-w-2xl text-base leading-relaxed text-sht-charcoal/70">{{ t('Catatan dan informasi terbaru yang berkaitan dengan kota Madinah, dilengkapi panduan yang relevan.') }}</p>
        </div>
        <NuxtLink :to="localePath('/hari-ini?city=madinah')" class="hidden shrink-0 pb-1 text-sm font-semibold text-sht-olive-dark hover:text-sht-olive lg:inline-flex lg:items-center lg:gap-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sht-gold">{{ t('Lihat Semua Madinah') }} <span class="text-sht-gold" aria-hidden="true">→</span></NuxtLink>
      </div>
