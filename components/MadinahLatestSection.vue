...
const { articles: latestArticles, pending, error } = await useMediaArticles({ city: 'MADINAH', limit: 4 })
</script>
  ...
  <section class="bg-sht-off-white py-16 sm:py-28" aria-labelledby="madinah-latest-heading">
    <div class="mx-auto max-w-container px-5 sm:px-6 lg:px-8">
      ...
      <div v-else-if="error && !latestArticles.length" class="mt-10">
        <CommonErrorState title="Artikel Madinah belum dapat dimuat" description="Silakan coba lagi nanti." :show-retry="false" />
      </div>

      <div v-else-if="!latestArticles.length" class="mt-10">
        <CommonEmptyState title="Belum ada artikel Madinah" description="Artikel terbaru Madinah akan muncul di sini." />
      </div>
        ...
        <div class="mt-10 hidden gap-x-5 gap-y-12 md:grid md:grid-cols-2 lg:mt-12 lg:grid-cols-4">
          <article v-for="article in latestArticles" :key="article.id" class="group min-w-0 snap-start md:flex md:flex-col">
            <NuxtLink :to="`/artikel/${article.slug}`" class="block focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sht-gold">
              <div class="overflow-hidden rounded-2xl bg-sht-stone aspect-[4/3]"><img :src="article.image || '/images/madinah-editorial.jpg'" :alt="article.imageAlt" loading="lazy" class="h-full w-full object-cover transition-transform duration-500 ease-out group-hover:scale-[1.02]" @error="onImageFallback($event, '/images/madinah-editorial.jpg')" /></div>
