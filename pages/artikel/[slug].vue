...
const route=useRoute();const slug=String(route.params.slug);const {article,pending,error}=await useMediaArticle(slug);if(error.value||!article.value)throw createError({statusCode:404,statusMessage:'Artikel tidak ditemukan'});const {articles:allArticles}=await useMediaArticles({limit:100});const current=article.value
...
async function selectFeedback(value:'helpful'|'not-helpful'){if(feedback.value||feedbackSubmitting.value)return;feedbackError.value='';feedbackSubmitting.value=true;const apiValue:MediaFeedbackValue=value==='helpful'?'HELPFUL':'NOT_HELPFUL';try{await submitMediaFeedback(slug,apiValue);feedback.value=value;localStorage.setItem(feedbackKey,apiValue)}catch(e:any){feedbackError.value=e?.data?.statusMessage||'Feedback belum berhasil dikirim. Silakan coba lagi.'}finally{feedbackSubmitting.value=false}}
</script>
